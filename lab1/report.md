...
1. 实验思路
    ...
    1. 小程序的运行由多个页面(pages)组成，每个页面的渲染与网页页面并无不同，都是由“微信版本”的三件套外加配置文件构成，即`.wxml` `.wxss` `.js`+ `.json`。

    2. 组件(`components`)可以理解为特殊的页面(`pages`)，同样由上述四件套渲染；区别在于它可以出现在多个页面当中。

    3. `app`类文件是全局文件，除了`.wxml`外都包含。

    4. `project` 类和`sitemap`类分别配置项目的运行环境（分内外）和配置搜索环境。一般不作修改。
...
完整可运行代码将放在个人Github仓库中。

---

## 二、 问题总结与体会

在将基础功能向高级界面演进的过程中，我遇到了三个非常典型的技术痛点，其排查与解决过程极大地深化了我对小程序框架的理解：

**第一个问题是 CSS 动画在初始化时的闪烁与白屏吞动画现象**。最开始，我直接在 WXSS 中给卡片绑定了带延迟的滑入动画。但在模拟器刷新时，由于白屏加载期 CSS 动画已经在后台默默计时，导致白屏结束后动画早已播放完毕，或者元素先闪现一下再缩回滑入。经过排查，我意识到需要将动画触发的控制权交给 JS：通过在 `onReady` 生命周期（即页面渲染完成、白屏消失的瞬间）将 `loaded` 设为 `true` 来动态挂载动画 Class，并配合 CSS 的 `animation-fill-mode: both`，这才实现了极其丝滑且无闪烁的滑入效果。

**第二个问题是开启沉浸式导航栏后的布局重叠与溢出**。开启隐藏原生导航栏后，页面顶部出现了漏白和内容挤压撞向系统胶囊的现象。我发现这是因为取消原生限制后，必须由开发者手动管理顶部安全距离。我首先将背景图设为全屏固定定位（`top: 0`）保证无缝覆盖，随后在 JS 的 `onLoad` 里通过 API 动态获取胶囊距离并赋值给 `headerPaddingTop`，精确地将页面头部推送到了胶囊下方。此外，针对单页小程序的上下误滑动问题，我在 `index.json` 中追加了 `"disableScroll": true`，锁死了页面拉伸，保持了原生应用的精致感。

**第三个问题是高亮背景下同色系元素的视觉隔离**。浅粉发色的头像在樱花树背景下显得非常模糊，缺乏焦点。我意识到不能使用简单的单色边框，于是设计了“暗桑葚紫内框 + 半透明发光外环”的双重边框，并在 CSS 中加入了 `saturate` 与 `contrast` 滤镜，不仅撕开了头像与背景的粘连感，还让角色宝蓝色的瞳孔更加吸睛。

这次实验让我受益匪浅。我不仅走通了从小程序创建到数据绑定的基本流程，更体会到了“满足基本需求”与“打磨优秀体验”之间的巨大差异。从毫米级的动态边距计算，到贝塞尔曲线的动画调优，这些细节让我对移动端 UI 开发有了更加深刻且直观的认识。